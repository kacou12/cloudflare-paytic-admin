  ...
  <div class="px-4 ">
    ...
    <header class="flex justify-between items-center">
      <div class="mb-6 space-y-1">
        ...
        <h1 class="text-xl font-bold font-merriweathersans leading-6 text-black">Gestion des collectes</h1>
        <p class="text-neutral-30 text-sm font-normal">Suivez, gérez et prévoyez les collectes de vos
          utilisateurs
        </p>
      </div>
      ...
      <section class="grid grid-cols-1 md:grid-cols-3  xl:grid-cols-5 gap-6  " v-if="isFetchedStats">
        <div class="xl:col-span-2 col-span-3 flex flex-col xs:flex-row xs:justify-center gap-6 ">
          ...
          <CommonStatisticCard class="xl:flex-1" :amount="transactionStatsData!.sum_txns.toString()"
            sub-title="L'essentiel des volumes collectés" :id="2" title="Volume collecté">
          </CommonStatisticCard>
        ...
        <div class="col-span-3 min-h-[234px] xl:h-full  flex justify-end " v-if="isFetchedStats">
          ...
          <CommonDoughnutCard :count_txns_failed="transactionStatsData!.count_txns_failed"
            :count_txns_success="transactionStatsData!.count_txns_success" :sum_txns="transactionStatsData!.sum_txns"
            :sum_txns_amount_group_by_provider="transactionStatsData!.sum_txns_amount_group_by_provider" class="w-full">
          </CommonDoughnutCard>
        </div>
      </section>
    ...
    <div class="flex xl:justify-between flex-col xl:flex-row my-4 ">
      <section class="xl:flex xl:flex-wrap grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3  gap-2 ">
        ...
        <CommonSelect v-model="operatorModel" :default-width="width >= 1366 ? 'w-fit' : 'w-full'" class=" w-full"
    ...
    <section class="my-5">
      ...
      <!-- <CommonDataTable :dynamic-width-columns="false" :total="collectionAndTransferData.length"
        :columns="collectionAndTransferColumn" :data="transactionsDashboardData!.items" @prev-page="prevPage" @next-page="nextPage">
      </CommonDataTable> -->


      <CommonDataTable :page-size="limitModel ? parseInt(limitModel) : 10" ref="my-table" :default-page="filters.page"
        :total="transactionsData?.total ?? 0" :columns="collectionAndTransferColumn"
...
const { isFetched, data: transactionsData, isFetching } = useTransactionsFiltersQuery(TransactionTypesEnum.Payout);
const { isFetched: isFetchedStats, data: transactionStatsData } = useTransactionStatsQuery(TransactionTypesEnum.Payout);
